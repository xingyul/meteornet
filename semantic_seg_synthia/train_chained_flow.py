...
import synthia_dataset_chained_flow
import class_mapping

parser = argparse.ArgumentParser()
parser.add_argument('--gpu', type=int, default=0, help='GPU to use [default: GPU 0]')
parser.add_argument('--data', default='processed_pc', help='Dataset dir [default: processed_pc]')
parser.add_argument('--data_chained_flow', default='chained_flow', help='Dataset for chained flow dir [default: chained_flow]')
...
DATA = FLAGS.data
DATA_CHAINED_FLOW = FLAGS.data_chained_flow
...
os.system('cp %s %s' % ('synthia_dataset_chained_flow.py', LOG_DIR)) # bkp of command file
...
TRAINVAL_DATASET = synthia_dataset_chained_flow.SegDataset(DATA, chained_flow_root=DATA_CHAINED_FLOW, filelist_name='data_prep/trainval_raw.txt', npoints=NUM_POINT, num_frames=NUM_FRAME, train=True)
...
def train():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Get model and loss
            pred, end_points = MODEL.get_model(pointclouds_pl, chained_flowed_pl, NUM_FRAME, is_training_pl, bn_decay=bn_decay)
        ...
        ops = {'pointclouds_pl': pointclouds_pl,
               'chained_flowed_pl': chained_flowed_pl,
               'labels_pl': labels_pl,
...
def get_batch(dataset, idxs, start_idx, end_idx, half=0):
    ...
    for i in range(bsize):
        pc, rgb, label, chained_flowed, labelweights, loss_mask, valid_pred_idx_in_full = dataset.get(idxs[i+start_idx], half)

        batch_data[i, :, :3] = pc
        batch_data[i, :, 3:] = rgb
        batch_chained_flowed[i] = chained_flowed
        batch_label[i] = label
        batch_mask[i] = loss_mask

    batch_labelweights = labelweights[batch_label]

    return batch_data, batch_chained_flowed, batch_label, batch_labelweights, batch_mask
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        for half in [0, 1]:
            ...
            batch_data, batch_chained_flowed, batch_label, batch_labelweights, batch_mask = \
                    get_batch(TRAINVAL_DATASET, train_idxs, start_idx, end_idx, half)

            feed_dict = {ops['pointclouds_pl']: batch_data,
                         ops['chained_flowed_pl']: batch_chained_flowed,
...
def eval_one_epoch(sess, ops, test_writer, dataset, epoch_cnt):
    ...
    for batch_idx in range(num_batches):
        ...
        for half in [0, 1]:
            cur_batch_data, cur_batch_chained_flowed, cur_batch_label, cur_batch_labelweights, cur_batch_mask = \
                    get_batch(dataset, test_idxs, start_idx, end_idx, half)
            if cur_batch_size == BATCH_SIZE:
                batch_data = cur_batch_data
                batch_chained_flowed = cur_batch_chained_flowed
                ...
                batch_chained_flowed[0:(cur_batch_size)] = cur_batch_chained_flowed
