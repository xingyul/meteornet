...
__global__ void gatherpointKernel(int b,int n,int m,const float * __restrict__ inp,const int * __restrict__ idx,float * __restrict__ out, int dim = 3){
  for (int i=blockIdx.x;i<b;i+=gridDim.x){
    for (int j=blockIdx.y*blockDim.x+threadIdx.x;j<m;j+=blockDim.x*gridDim.y){
      int a=idx[i*m+j];
      for (int k = 0; k < dim; k ++) {
          out[(i*m+j)*dim+k]=inp[(i*n+a)*dim+k];
      }
    }
  }
}
...
void gatherpointLauncher(int b,int n,int m,const float * inp,const int * idx,float * out,int dim=3){
  gatherpointKernel<<<dim3(2,8,1),512>>>(b,n,m,inp,idx,out,dim);
}
